...
class Tree:
  ...
  def classify(self, classificationData):
    ...
    while not currentNode.isleaf:
        currentAttr = currentNode.attr
        currentNode = currentNode.children[classificationData[currentAttr]]
...
def getAttributeCounts(examples,attrName,attrValues,className):
    ...
    for attrValue in attrValues:
        filtered = filter(lambda x: x[attrName] == attrValue, examples)
        classCount = getClassCounts(filtered, className)
...
def setEntropy(classCounts):
    ...
    for classCount in classCounts:
        pk = float(classCount) / totalCount
        currentEntro = pk * log(1 / pk, 2)
        out += currentEntro
    return out
...
def remainder(examples,attrName,attrValues,className):
    ...
    for value in attrValues:
        attributeCount = attributeCounts[value]
        attributeCountArr = attributeCount.values()
        remainder += float(sum(attributeCountArr)) / total * setEntropy(attributeCountArr)
    return remainder
...
def giniIndex(classCounts):
    ...
    for value in classCounts:
        gini -= float(value) / total * float(value) / total
    return gini
...
def giniGain(examples,attrName,attrValues,className):
    ...
    for value in attrValues:
        attributeCount = attributeCounts[value]
        attrbuteCountArr = attributeCount.values()
        gain += giniIndex(attrbuteCountArr) * sum(attrbuteCountArr) / total
    if gain == 0:
        return sys.maxint
    return 1 / gain
...
def makeSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc):
    ...
    if len(examples) == 0: 
    ...
    elif len(set(map(lambda x: x[className], examples))) == 1:
        return LeafNode(map(lambda x: x[className], examples)[0])
        ...
        for attribute in remainingAttributes:
            theAttrValues = attributeValues[attribute]
            currentScore = gainFunc(examples, attribute, theAttrValues, className)
            if currentScore > score:
                argMax = attribute
                score = currentScore
        tree = Node(argMax)
        for v in attributeValues[argMax]:
            newExamples = getPertinentExamples(examples, argMax, v)
            #print remainingAttributes
            #print argMax
            newRemainingAttributes = filter(lambda x: x != argMax, remainingAttributes)
            newDefault = getMostCommonClass(examples, className)
            subtree = makeSubtrees(newRemainingAttributes, newExamples, attributeValues, className, newDefault, setScoreFunc, gainFunc)
